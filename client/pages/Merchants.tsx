      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Daftar Merchant QRIS</h1>
            <p className="text-slate-500 text-sm font-medium">Kelola dan verifikasi merchant yang terdaftar dalam sistem</p>
          </div>
            ...
            <div key={idx} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all group">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                {stat.change.includes('+') ? (
                  <span className="text-emerald-500">{stat.change.split(' ')[0]}</span>
                ) : stat.change.includes('%') ? (
                  <span className="text-blue-500">{stat.change.split(' ')[0]}</span>
                ) : (
                  <span className="text-amber-500">{stat.change.split(' ').slice(0,2).join(' ')}</span>
                )}
                <span className="text-slate-400">{stat.change.split(' ').slice(stat.change.includes('Perlu') ? 2 : 1).join(' ')}</span>
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-4 flex-1 flex-wrap">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Kategori:</span>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Wilayah:</span>
              <Select defaultValue="all">
                <SelectTrigger className="h-10 w-44 rounded-xl bg-slate-50 border-transparent focus:ring-0">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            <h3 className="text-lg font-extrabold text-slate-900">Daftar Merchant</h3>
            <div className="flex items-center gap-2">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">ID QRIS</th>
                  ...
                  <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Transaksi</th>
                  ...
                  <tr key={idx} className="hover:bg-slate-50/30 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900">{merchant.nama}</p>
                          <p className="text-[10px] font-medium text-slate-400 leading-tight">{merchant.alamat}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-sm font-mono font-medium text-slate-600">{merchant.idQris}</td>
                    <td className="px-6 py-5">
                      <span className={cn("px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border", merchant.kategoriColor)}>
                        {merchant.kategori}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-sm font-semibold text-slate-600">{merchant.wilayah}</td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-2">
                        <div className={cn("w-2 h-2 rounded-full", merchant.statusColor)} />
                        <span className="text-xs font-bold text-slate-700">{merchant.status}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-sm font-bold text-slate-900">{merchant.transaksi}</td>
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-center gap-2">
                        <button className="p-2 text-[#1E6CF6] hover:bg-blue-50 rounded-lg transition-all" title="View">
                        ...
                        {merchant.status === "Verifikasi" ? (
                          <>
                            ...
                            <button className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-all" title="Reject">
                              <X className="w-4 h-4" />
                            </button>
                          </>
                        ) : (
          ...
          <div className="p-6 border-t border-slate-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="h-9 rounded-lg border-slate-200 text-slate-600 font-bold hover:bg-slate-50 gap-1 px-3">
